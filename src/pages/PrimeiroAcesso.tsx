...
import { ArrowRight, Check, Loader2, Lock, Trophy } from 'lucide-react'
...
export function PrimeiroAcesso() {
  ...
  async function concluirSenha() {
    setErro(null)
    if (novaSenha.length < 8) { setErro('A senha deve ter pelo menos 8 caracteres.'); return }
    if (!/\d/.test(novaSenha)) { setErro('A senha deve conter pelo menos 1 número.'); return }
    ...
    await supabase.from('profiles').update({ senha_trocada: true }).eq('id', user!.id)
    setLoading(false)
    setStep(1)
  }
  ...
  const STEPS = [
    {
      ...
      subtitulo: 'Para começar, crie sua senha pessoal.',
    },
    {
      titulo: 'Apostas especiais',
      subtitulo: 'Valem pontos bônus no final da Copa. Escolha com cuidado!',
    },
    {
      titulo: 'Tudo certo!',
        ...
        <div className="mb-8 flex items-center justify-center gap-2">
          {[0, 1, 2].map((i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              {i < 2 && <div className={`h-0.5 w-10 ${i < step ? 'bg-brasil-green' : 'bg-white/10'}`} />}
            </div>
          ))}
        </div>
            ...
            >
              ...
              {step === 0 && (
                <div className="space-y-4">
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-zinc-300">
                      <Lock className="mr-1.5 inline h-4 w-4" /> Nova senha
                    </label>
                    <input
                      type="password"
                      value={novaSenha}
                  ...
                  <div>
                    <label className="mb-1.5 block text-sm font-medium text-zinc-300">
                      <Lock className="mr-1.5 inline h-4 w-4" /> Confirmar senha
                    </label>
                    <input
                      type="password"
