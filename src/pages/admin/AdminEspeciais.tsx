...
export function AdminEspeciais() {
  ...
  useEffect(() => {
    async function carregar() {
      ...
      if (jogos && jogos.length > 0) {
        ...
        if (melhor) setMelhorDefesaAuto(`${melhor[0]} (${melhor[1]} gols sofridos)`)
      }
  ...
  async function salvar() {
    ...
    setSalvando(false)
    setFeedback(error ? 'Erro ao salvar.' : 'Salvo com sucesso!')
    setTimeout(() => setFeedback(null), 3000)
  }

  async function finalizar() {
    if (!window.confirm('Finalizar a Copa? Isso calcula os pontos especiais de todos os participantes.')) return
    setFinalizando(true)
    setFeedback(null)
    ...
    setFeedback(error ? `Erro ao calcular pontos: ${error.message}` : 'Copa finalizada! Pontos especiais calculados.')
  ...
  return (
    <>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-zinc-100">Escolhas Especiais</h1>
          <p className="mt-1 text-sm text-zinc-500">Resultados finais da Copa — define pontos bônus dos participantes</p>
        </div>
        {form.finalizado && (
          <span className="inline-flex items-center gap-1.5 rounded-full bg-green-900/30 px-3 py-1 text-sm font-medium text-green-400">
            <Trophy className="h-4 w-4" /> Copa Finalizada
          </span>
        )}
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
          <h2 className="mb-4 font-semibold text-zinc-200">Resultados</h2>
          <div className="space-y-4">
            {[
              { key: 'campeao', label: '🏆 Campeão', pts: 30 },
              { key: 'vice_campeao', label: '🥈 Vice-campeão', pts: 15 },
              { key: 'terceiro', label: '🥉 3º lugar', pts: 10 },
              { key: 'artilheiro', label: '⚽ Artilheiro', pts: 20 },
              { key: 'melhor_jogador', label: '🌟 Melhor jogador', pts: 15 },
            ].map(({ key, label, pts }) => (
              <div key={key}>
                ...
                <input
                  ...
                  placeholder={key === 'artilheiro' || key === 'melhor_jogador' ? 'Nome do jogador' : 'Nome da seleção'}
                  className={INPUT}
            ...
            <div>
              <div className="mb-1 flex items-center justify-between">
                <label className="text-sm font-medium text-zinc-300">
                  <Shield className="mr-1 inline h-3.5 w-3.5" /> Melhor defesa
                </label>
                <span className="text-xs text-zinc-500">calculada automaticamente</span>
              </div>
              {melhorDefesaAuto && (
                <div className="mb-2 flex items-center gap-2 rounded-lg border border-zinc-700 bg-zinc-800/50 px-3 py-2 text-sm text-zinc-400">
                  ...
                  Baseado nos jogos: <span className="font-medium text-zinc-200">{melhorDefesaAuto}</span>
                </div>
              )}
              <input
                type="text"
                ...
                placeholder="Confirmar / ajustar seleção"
                className={INPUT}
                disabled={form.finalizado}
              />
            </div>
          ...
          {feedback && (
            <motion.p
              ...
              className={`mt-4 rounded-lg px-3 py-2 text-sm ${
                feedback.startsWith('Erro') ? 'bg-red-500/10 text-red-400' : 'bg-green-500/10 text-green-400'
              }`}
            >
              {feedback}
            </motion.p>
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
          <h2 className="mb-4 font-semibold text-zinc-200">Tabela de pontuação</h2>
          <div className="space-y-3">
            {[
              { label: '🏆 Campeão correto', pts: 30 },
              { label: '⚽ Artilheiro correto', pts: 20 },
              { label: '🌟 Melhor jogador correto', pts: 15 },
              { label: '🥈 Vice-campeão correto', pts: 15 },
              { label: '🥉 3º lugar correto', pts: 10 },
            ].map(({ label, pts }) => (
              <div key={label} className="flex items-center justify-between rounded-lg bg-zinc-800 px-4 py-3">
          ...
          <p className="mt-4 text-xs text-zinc-600">
            Clique em "Finalizar Copa" após preencher todos os campos. Isso recalcula os pontos especiais de todos os participantes automaticamente.
          </p>
        </div>
      </div>
    </>
