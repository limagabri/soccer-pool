...
function statusConvite(c: Convite): { label: string; cls: string } {
  if (c.usado) return { label: 'Usado', cls: 'bg-zinc-800 text-zinc-500' }
  if (new Date(c.expires_at) < new Date()) return { label: 'Expirado', cls: 'bg-red-900/30 text-red-500' }
  return { label: 'Pendente', cls: 'bg-amber-900/30 text-amber-400' }
}
...
export function AdminConvites() {
  ...
  async function gerarConvite() {
    ...
    setGerando(false)
    if (error) {
      setErro(error.message.includes('unique') ? 'Já existe um convite para este e-mail.' : 'Erro ao gerar convite.')
      return
    }
  ...
  return (
    <>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-zinc-100">Convites</h1>
      ...
      <div className="mb-6 rounded-xl border border-zinc-800 bg-zinc-900 p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-400">Nome de usuário</label>
            <input
              type="text"
              value={nome}
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-400">E-mail</label>
            <input
              type="email"
              value={email}
        ...
        {novoConvite && (
          ...
          >
            <p className="mb-3 text-sm font-semibold text-green-400">
              Convite gerado para <strong>{novoConvite.nome}</strong> ({novoConvite.email})
            </p>
            ...
            <p className="mt-2 text-xs text-zinc-600">Válido por 7 dias.</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-semibold text-zinc-400 uppercase tracking-wider">Convites gerados</h2>
          ...
          <table className="w-full min-w-[540px] text-sm">
            <thead>
              <tr className="border-b border-zinc-800 text-left text-xs tracking-wider text-zinc-500 uppercase">
                ...
                <th className="px-3 py-3 text-center">Gerado em</th>
