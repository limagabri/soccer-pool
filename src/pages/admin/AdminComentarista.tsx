...
const TIPOS = [
  { value: 'rodada_grupos', label: 'Fase de Grupos', rodadas: [1, 2, 3] },
  { value: 'rodada_32avos',   label: '32-avos de Final', rodadas: [] },
  { value: 'rodada_oitavas',  label: 'Oitavas de Final', rodadas: [] },
  { value: 'rodada_quartas',  label: 'Quartas de Final', rodadas: [] },
  { value: 'rodada_semis',    label: 'Semifinais',        rodadas: [] },
  { value: 'rodada_final',    label: 'Final da Copa',     rodadas: [] },
]
...
export function AdminComentarista() {
  ...
  async function gerar() {
    ...
    if (!res.ok) {
      ...
      setErro(err.error ?? 'Erro ao gerar comentário.')
      setGerando(false)
      return
    }
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-2xl font-bold text-zinc-100">Comentarista IA — Seu Zé</h1>

      {/* Generator */}
      <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-6 space-y-4">
        <h2 className="text-sm font-semibold tracking-wider text-zinc-400 uppercase">Gerar Comentário</h2>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {tipoAtual.rodadas.length > 0 && (
            <select
              ...
              className="rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 text-sm text-zinc-200 outline-none focus:border-green-600"
            >
              {tipoAtual.rodadas.map(r => <option key={r} value={r}>Rodada {r}</option>)}
            </select>
          )}
          ...
          >
            ...
            {preview ? 'Regenerar' : 'Gerar com IA'}
          </button>
        </div>
      ...
      {/* History */}
      <div className="space-y-3">
        <h2 className="text-sm font-semibold tracking-wider text-zinc-400 uppercase">Histórico</h2>
        {historico.length === 0 ? (
          <p className="text-sm text-zinc-600">Nenhum comentário gerado ainda.</p>
        ) : (
          historico.map(c => (
